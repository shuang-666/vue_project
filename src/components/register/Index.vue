<template>
  <div class="lodin_wrapper">
    <div class="login_container">
      ...
      <div class="login_item_container">
        <input placeholder="请输入手机号" v-model="mobel" />
        <input type="password" placeholder="请输入密码" v-model="pwd" />
        <div class="yanzhengma">
          <button class="button_one" @click="password">找回密码</button>
          <button @click="toResiger" class="button_two">注册</button>/
          <button @click="toResiger">验证码登录</button>
        </div>
      </div>
    </div>
    <!-- button按钮 -->
...
.login_container .login_icon img {
  width: 465px;
  height: 105px;
  border-radius: 0px;
}
...
.login_item_container input::placeholder {
  color: #ccc;
  font-size: 31px;
}
.login_item_containerr .yanzhengma {
  align-items: center;
  width: 100%;
  margin-top: 30px;
}
